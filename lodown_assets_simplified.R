...
# 2000 only
censo_cat <- subset( get_censo_cat , state == 'sp2_rm10')
# 'sp2_rm10'
# 'sp00'
...
for( this_state in seq( nrow( censo_cat ) ) ){
  ...
  this_censo_df <-  this_censo_dom_df
...
write.csv( censo_df, file = file.path( path.expand( datasets.path ), 'censo_assets' ,'sp10' ) )
